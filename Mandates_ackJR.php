...
Traza(" MANDATES_ACK", $bodyRequest);
$results = true;

foreach ($params as $mandato) {
    $idMandate = $mandato->IdMandato;
    $resultCode = $mandato->ResultCode;
    $resulText = $mandato->ResultText;

    //Si viene Code == 0 actualizamos el mandato
    if ($resultCode == 0) {
        $response = $obj->setSincronized($idMandate);
        if(!isMandatesynced($obj, $idMandate)) {
            $results = false;
        }
    }
}
...
function get_obj() {
    $object = new model_mandatosJR_class;
    return $object;
}
...
//Comprueba que efectivamente el estado de sincronizacion de un mandato se encuentra a -1
function isMandatesynced($obj, $idMandate) {
    $status = $obj->getSyncStatus($idMandate);
    $sync = 0;
    if (count($status) > 1) {
...
function print_json($status, $mensaje) {
    ...
    Traza(" MANDATES_ACK_RESPUESTA", json_encode($response, JSON_UNESCAPED_UNICODE));
}
?>
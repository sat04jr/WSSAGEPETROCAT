...
$select = "CONVERT(char(36), PedidoCliente.BLM_MovCompraBase) as IdCompra,  ComprasBases.IdDelegacion AS CodigoCanal, Clientes.CifDni, ComprasBases.CodigoArticulo, PedidoCliente.DescripcionArticulo,
                         ...
                         CASE WHEN PedidoCliente.UnidadesReales =0 THEN ComprasBases.Litros ELSE PedidoCliente.UnidadesReales END  AS BLM_Unidades15G, 
						 RIGHT(YEAR(PedidoCliente.FechaSuministro),2)+ CAST(PedidoCliente.NumeroCargaAutorizada AS VARCHAR(12)) NumeroCargaAutorizada, 
...
$select2 = "CONVERT(char(36), ComprasBases.BLM_IDCompraBases) AS IdCompra, ComprasBases.IdDelegacion AS CodigoCanal, Clientes.CifDni, PedidoClienteBases.CodigoArticulo, PCA.DescripcionArticulo, 
						 ...
						 CASE WHEN PedidoCliente.UnidadesReales =0 THEN ComprasBases.Litros ELSE PedidoCliente.UnidadesReales END AS BLM_Unidades15G, RIGHT(YEAR(PCA.FechaSuministro),2)+ CAST(PCA.NumeroCargaAutorizada AS VARCHAR(12)) NumeroCargaAutorizada , '' AS ARC, 
...
$where2 = " (ComprasBases.BLM_IdPedidoVenta <> '00000000-0000-0000-0000-000000000000') AND (PedidoCliente.BLM_MovCompraBase <> '00000000-0000-0000-0000-000000000000') AND (YEAR(PCA.FechaSuministro) 
                         >= 2020) AND (BLM_DatosNif.BLM_CodigoClienteHeptan <> '') 
						AND PCA.NumeroCargaAutorizada<>0
						AND (CASE WHEN PCA.UnidadesReales = 0 THEN ComprasBases.Litros ELSE PCA.UnidadesReales END > 0)";
...
$select3 = "CONVERT(char(36), ComprasBases.BLM_IDCompraBases) AS IdCompra, ComprasBases.IdDelegacion AS CodigoCanal, Clientes.CifDni, ComprasBases.CodigoArticulo, PCA.DescripcionArticulo, 
						 ...
						 CASE WHEN PCA.UnidadesReales =0 THEN ComprasBases.Litros ELSE PCA.UnidadesReales END AS BLM_Unidades15G, RIGHT(YEAR(pedidocliente.FechaSuministro),2)+ CAST(pedidocliente.NumeroCargaAutorizada AS VARCHAR(12)) NumeroCargaAutorizada, '' AS ARC, 
...
//Traza(" PURCHASEORDERHEPTAN", $query);
    ...
    foreach ($data as $key => $purchaseOrder) {
		...
		*/
		...
		if($purchaseOrder['NumeroCargaAutorizada'] != '') {
			$data[$key]['NumeroCargaAutorizada'] = $purchaseOrder['NumeroCargaAutorizada'];//intVal($purchaseOrder['NumeroCargaAutorizada']);	
		}
		else

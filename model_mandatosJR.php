<?php 
 ...
 class model_mandatosJR_class extends db_model {
...
//Marca el mandato como sincronizado
    function setSincronized($idMandato) {
		echo sprintf("BEGIN TRY 
        UPDATE %s SET %s WHERE BLM_Heptan_IdMandato = CONVERT(uniqueidentifier, '%s') OR IdMandatoUnico = CONVERT(uniqueidentifier, '%s')
        END TRY
        BEGIN CATCH
        PRINT 0
        END CATCH",
        ...
        return $this->set_query(sprintf("BEGIN TRY 
        UPDATE %s SET %s WHERE BLM_Heptan_IdMandato = CONVERT(uniqueidentifier, '%s') OR IdMandatoUnico = CONVERT(uniqueidentifier, '%s')
        END TRY
        BEGIN CATCH
        PRINT 0
...
function getSyncStatus($idMandato) {
	return $this->get_query(sprintf("BEGIN TRY 
	SELECT BLM_Heptan_sync FROM %s WHERE BLM_Heptan_IdMandato = CONVERT(uniqueidentifier, '%s') OR IdMandatoUnico = CONVERT(uniqueidentifier, '%s')
	END TRY
	BEGIN CATCH
	PRINT 0
	END CATCH",

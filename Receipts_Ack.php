...
Traza(" RECEIPTS_ACK", $bodyRequest);
// print_r($params);die;
$results = true;

foreach ($params as $efecto) {
    $id = $efecto->IdEfecto;
    $resultCode = $efecto->ResultCode;
    $resultText = $efecto->ResultText;

    if ($resultCode == 0 || $resultCode == -112 || $resultCode == -108) {
		$obj->setSincronized($id);
		if (!isEfectoSynced($obj, $id)) {
			$results = false;
		}
	}
}
...
function isEfectoSynced($obj, $idEfecto) {
    $status = $obj->getSyncStatus($idEfecto);
    $sync = 0;
    if (count($status) > 1) {
...
function print_json($status, $mensaje) {
    ...
    Traza(" RECEIPTS_ACK_RESPUESTA", json_encode($response, JSON_UNESCAPED_UNICODE));
}
?>
...
class model_locations_heptan_class extends heptan_db_model {
    ...
    function getUltimaDescarga($LineasPosicion) {
        $response = $this->set_query(sprintf("SELECT TOP 1 CONVERT(char(36), LineasPosicion) as LineasPosicion FROM WSHeptan_Descargas where BLM_WSProcesado=-1 AND LineasPosicion = '%s'",
            $LineasPosicion));
        $Campo="";

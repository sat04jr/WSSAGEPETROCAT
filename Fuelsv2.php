...
Traza(" FUELSV2", "heptanStatus=".$heptanStatus." block=".$block." tipoTarifa=".$tipoTarifa);
...
switch ($tipoTarifa) {
    ...
    case 1:
		...
		$fromTarifas3 = "Tarifas  WITH (nolock) 
		INNER JOIN TarifaPrecio WITH (nolock) ON Tarifas.CodigoEmpresa = TarifaPrecio.CodigoEmpresa 
		AND Tarifas.Tarifa = TarifaPrecio.Tarifa 
		inner join  BLM_PR120_ConversionArticulos WITH (nolock) ON TarifaPrecio.CodigoEmpresa = BLM_PR120_ConversionArticulos.CodigoEmpresa 
		AND TarifaPrecio.CodigoArticulo = BLM_PR120_ConversionArticulos.CodigoArticulo AND BLM_PR120_ConversionArticulos.BLM_EnvioHeptan = - 1
		AND -1 = Tarifas.BLM_TarifaCliHeptan";
        $where .= " AND FechaInicio <= '".$fechaString."' AND Year(FechaInicio) >= 2017";
		break;
    default:
		print_json(400, 1, "Error TipoTarifa. " .error_get_last()['message'] .'. ' .error_get_last()['file'] .'. ' .error_get_last()['line'], null);
...
function print_json($status, $queryStatus, $mensaje, $data) {
    ...
    Traza(" FUELSV2_RESPUESTA", json_encode($response, JSON_UNESCAPED_UNICODE));
}
?>
...
include 'model_saleOrder_heptan.php';
	...
	if ($seguirbuscando==1){
	   ...
	   $depositos=mssql_query($depositosQuery);
		...
		if ($ExisteDeposito=='') {
			...
			if ($IdPedido == "") {
				if (substr($SeriePedido,-2)=="00"){
					//ACTUALIZAR CONTADOR SOLO PARA SERIE 00
					$ExisteContador = -1;
					$contadorQuery="SELECT N_Pedido FROM ContadoresBases WITH (NOLOCK)
									 where CodigoEmpresa=3 AND Ejercicio=".$EjercicioPedido."
									AND IdDelegacion='".$IdDelegacion."'";
				   $contador=mssql_query($contadorQuery);
					while ($row = mssql_fetch_array($contador, MSSQL_ASSOC)){
						$row2 = array_map('utf8_encode', $row);
						$ExisteContador = $row2['N_Pedido'];
					}

					if($ExisteContador==-1){
						$contadorQuery="INSERT INTO ContadoresBases (CodigoEmpresa,Ejercicio,IdDelegacion, N_Pedido)
							VALUES(3,$EjercicioPedido,'$IdDelegacion',$NumeroPedido)";
						$contador=mssql_query($contadorQuery);

						
					} else {
						$contadorQuery="UPDATE ContadoresBases SET
...
/*				if ($BLM_PedidoWeb==-1 && $Estado==1) {
					//ENVIAR ESTADO A NATEEVO
					$PedidoNateevo[] = [
						'EjercicioPedido' => $EjercicioPedido,
						'SeriePedido' => $SeriePedido,
						'NumeroPedido' => $NumeroPedido
					];
					$json=json_encode($PedidoNateevo, JSON_UNESCAPED_SLASHES);
					$postdata = $json;//http_build_query($json);
	//				print_r($postdata);
					$ch = curl_init();
	//				curl_setopt($ch,CURLOPT_URL, 'http://77.246.187.130:7777/wsnateevo/pedidoestado.php');
					curl_setopt($ch,CURLOPT_URL, 'localhost:7676/wsnateevo/pedidoestado.php');
...
function get_obj() {
    $object = new model_saleOrder_heptan_class;
    return $object;
}

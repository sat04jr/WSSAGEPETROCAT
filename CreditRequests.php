...
include 'model_CreditRequests.php';
...
$select = "CONVERT(char(36), BLM_SolicitudesRiesgo.IdRiesgo) AS IDRiesgo, BLM_DatosNif.CifDni, BLM_SolicitudesRiesgo.BLM_ImporteSolicitado AS RiesgoCC, BLM_SolicitudesRiesgo.BLM_ImporteConcedido AS RiesgoCCConcedido, BLM_SolicitudesRiesgo.FechaSolicitud, BLM_SolicitudesRiesgo.BLM_StatusSolicitudRiesgo AS StatusSolicitudRiesgo, BLM_SolicitudesRiesgo.BLM_EMailSolicitante AS EMailSolicitante, BLM_SolicitudesRiesgo.DiasPrimerPlazo, BLM_SolicitudesRiesgo.DiasFijos1, BLM_SolicitudesRiesgo.FormadePago, BLM_SolicitudesRiesgo.RazonSocial, BLM_SolicitudesRiesgo.Domicilio, BLM_SolicitudesRiesgo.Codigopostal, BLM_SolicitudesRiesgo.Municipio, BLM_SolicitudesRiesgo.Provincia, BLM_SolicitudesRiesgo.Telefono ";

$tablasQuery = "BLM_DatosNif INNER JOIN BLM_SolicitudesRiesgo ON BLM_DatosNif.CifDni = BLM_SolicitudesRiesgo.CifDni ";
...
if ($IDRiesgo) {
    $where .= " AND (BLM_SolicitudesRiesgo.IdRiesgo = CONVERT(uniqueidentifier, '".$IDRiesgo."'))";
} else {
    ...
    if ($cifDni != '') {
        ...
        $where .= " AND (BLM_SolicitudesRiesgo.BLM_Heptan_sync = 0) AND (BLM_SolicitudesRiesgo.BLM_StatusSolicitudRiesgo = 'A' OR BLM_SolicitudesRiesgo.BLM_StatusSolicitudRiesgo = 'R') ";
		}
	}
...
function get_obj() {
	$object = new model_riesgos_class;
	return $object;
}
...
function dateTransform($data) {
    foreach ($data as $key => $riesgos) {
		//FechaSolicitud
        if($riesgos['FechaSolicitud'] != '') {
            $strDate = strtotime($riesgos['FechaSolicitud']);
            $badDate = date('d/m/Y', $strDate );
                $data[$key]['FechaSolicitud'] = $badDate;
        }
		
		//RiesgoCC (double)
		if($riesgos['RiesgoCC'] != '') {
			$data[$key]['RiesgoCC'] = doubleVal($riesgos['RiesgoCC']);	
		}
		else
			if($riesgos['RiesgoCC'] != 0) {
				$data[$key]['RiesgoCC'] = 0;
			}
		...
		if($riesgos['RiesgoCCConcedido'] != '') {
			...
			if($riesgos['RiesgoCCConcedido'] != 0) {
				...
				//DiasFijos1
		if($riesgos['DiasFijos1'] != '') {
			$data[$key]['DiasFijos1'] = intVal($riesgos['DiasFijos1']);	
		}
		else
			$data[$key]['DiasFijos1'] = 0;
		
		//DiasPrimerPlazo
		if($riesgos['DiasPrimerPlazo'] != '') {
			$data[$key]['DiasPrimerPlazo'] = intVal($riesgos['DiasPrimerPlazo']);	
		}
		else

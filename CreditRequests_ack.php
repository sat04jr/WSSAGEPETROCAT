...
$obj = get_obj();
$obj->entity = 'BLM_SolicitudesRiesgo';
$obj->data = 'BLM_Heptan_sync = -1';

//Capturamos las variables
$params = json_decode($bodyRequest);
Traza(" CREDITREQUESTS_ACK", $bodyRequest);
$results = true;
...
foreach ($params as $riesgo) {
    ...
    if ($resultCode == 0) {
        $response = $obj->setSincronized($idRiesgo);
        if(!isRiesgosynced($obj, $idRiesgo)) {
            $results = false;
        }
    }
}
...
function print_json($status, $mensaje) {
    ...
    Traza(" CREDITREQUESTS_ACK_RESPUESTA", json_encode($response, JSON_UNESCAPED_UNICODE));
}
?>
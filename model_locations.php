<?php 
 ...
 class model_locations_class extends db_model {
...
//Marca el cliente como sincronizado
    function setSincronized($id) {
        return $this->set_query(sprintf("UPDATE %s SET %s WHERE BLM_Heptan_sync=0 AND BLM_Heptan_idDeposito = CONVERT(uniqueidentifier, '%s')",
            $this->entity,
            $this->data,
            $id
        ));
...
function getSyncStatus($id) {
	return $this->get_query(sprintf("select BLM_Heptan_sync FROM %s WHERE BLM_Heptan_idDeposito = CONVERT(uniqueidentifier, '%s')",
            $this->entity,
            $id
        ));
}

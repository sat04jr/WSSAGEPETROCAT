<?php 
 ...
 class model_cartera_class extends db_model {
...
//Marca el cliente como sincronizado
    function setSincronized($id) {
		//echo sprintf("UPDATE %s SET %s WHERE MovCartera = CONVERT(uniqueidentifier, '%s')", $this->entity, $this->data, $id);
		//die;
        return $this->set_query(sprintf("UPDATE %s SET %s WHERE MovCartera = CONVERT(uniqueidentifier, '%s')",
            $this->entity,
            $this->data,
            $id
        ));
...
function getSyncStatus($id) {
	return $this->get_query(sprintf("select BLM_Heptan_sync FROM %s WHERE MovCartera = CONVERT(uniqueidentifier, '%s')",
            $this->entity,
            $id
        ));
}

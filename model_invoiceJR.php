<?php 
 ...
 class model_invoiceJR_class extends db_model {
...
//Marca el mandato como sincronizado
    function setSincronized($EjercicioFactura, $SerieFactura, $NumeroFactura) {
        return $this->set_query(sprintf("UPDATE %s SET %s WHERE EjercicioFactura = '%s' AND SerieFactura = '%s' AND NumeroFactura = '%s'",
            $this->entity,
            $this->data,
            $EjercicioFactura,
            $SerieFactura,
...
function getSyncStatus($EjercicioFactura, $SerieFactura, $NumeroFactura) {
	return $this->get_query(sprintf("SELECT BLM_Heptan_sync FROM %s WHERE EjercicioFactura = '%s' AND SerieFactura = '%s' AND NumeroFactura = '%s'",
        $this->entity,
        $EjercicioFactura,
        $SerieFactura,
		$NumeroFactura

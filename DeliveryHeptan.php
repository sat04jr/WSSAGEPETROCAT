...
foreach ($params as $delivery) {
	...
	$SuPedido = utf8_decode(str_replace("'","''",$delivery->SuPedido));
	...
	$Chofer = $BLM_AditivoExcelentChofer;
	if ($BLM_AditivoExcelentChofer>32767) {
	...
	$datacadena="";
	$CodigoCadena="";
    $query="SELECT CodigoEmpresa, CodigoCadena_ FROM Cadenas_ WHERE CodigoEmpresa=1 AND CodigoCadena_='".$PeriodoFacturacion."' AND CodigoCadena_<>''";
    //	echo $query;die;
    ...
    if(mssql_num_rows($registro) > 0) {
        $CodigoCadena = $datacadena[0]["CodigoCadena_"];
    }
...
/*    $AgruparAlbaranes=0;
  ...
  */  
	...
	if ($TipoDomicilioFactura==0) {
	    ...
	    $query="SELECT        Heptan_Descargas.DomicilioDescarga, Heptan_Descargas.CodigoMunicipioDescarga, Heptan_Descargas.MunicipioDescarga, 
				Heptan_Descargas.CodigoPostalDescarga, 
                         Heptan_Descargas.CodigoProvinciaDescarga, ISNULL(Provincias.Provincia, '') AS Provincia, Naciones.CodigoNacion, Naciones.Nacion
			FROM            Heptan_Descargas LEFT OUTER JOIN
                         Provincias ON Heptan_Descargas.CodigoProvinciaDescarga = Provincias.CodigoProvincia CROSS JOIN
                         Naciones WHERE Heptan_Descargas.BLM_IdDescarga='".$IdDescarga."'";
	    //	 echo $query;die;
	    $registro = mssql_query($query);
	    while ($row = mssql_fetch_array($registro, MSSQL_ASSOC)){
	        $row2 = array_map('utf8_encode', $row);
	    ...
	    // Si la cantidad de elementos que trae el array de $data es igual a 0 entra en este condicional
	    if(mssql_num_rows($registro) > 0) {
	        $DomicilioFactura = utf8_decode(str_replace("'","''",$data[0]["DomicilioDescarga"]));
	        $CodigoPostalFactura = $data[0]["CodigoPostalDescarga"];
	        $CodigoMunicipioFactura= $data[0]["CodigoMunicipioDescarga"];
	        $MunicipioFactura= utf8_decode(str_replace("'","''",$data[0]["MunicipioDescarga"]));
	        $ProvinciaFactura = utf8_decode(str_replace("'","''",$data[0]["Provincia"]));
	        $CodigoAutonomiaFactura = 0;
	        $CodigoPaisFactura = $data[0]["CodigoNacion"];
	    }
	...
	$NumeroPedido = 0;
	$DevolucionWallet=0;
	if ($IdPedido<>"" && $IdPedido<>"00000000-0000-0000-0000-000000000000") {
	    $query="SELECT EjercicioPedido, SeriePedido, NumeroPedido, BLM_NombreAgenteWeb FROM PedidoClienteBases WHERE CodigoEmpresa=3 and BLM_IdPedido='".$IdPedido."'";
	    //echo $query;die;
	    $registro = mssql_query($query);
	    while ($row = mssql_fetch_array($registro, MSSQL_ASSOC)){
	        $row2 = array_map('utf8_encode', $row);
	    ...
	    if(mssql_num_rows($registro) > 0) {
			...
			if ($data[0]["BLM_NombreAgenteWeb"]=="Directe pagado") {
				$DevolucionWallet=-1;
			}
	    }
	}
...
//echo "bloqueo".$Bloqueo;
	...
	if ($IdAlbaran == "") {
	    ...
	    if(mssql_num_rows($registro) > 0) {
	        if ($data[0]["StatusFacturado"]==0) {
				...
				$data="";
				//BUSCAR LA LINEA Y DEVOLVER EL IdAlbaran
		...
		if(mssql_num_rows($registro) > 0) {
			...
			if($Albaranfacturado==-1) {
				...
				'IdAlbaran' => $IdAlbaran,
				'ResultCode' => 0,
				'ResultText' => ''.$tiporesult.' Albaran facturado'];
				continue;
				//print_json($results);
				//$obj = null;
				//die;
		...
		if (count($NumeroAlbaranResult) > 0 && $NumeroAlbaranResult[0] && array_key_exists('NumeroAlbaran', $NumeroAlbaranResult[0])) {
			...
			BLM_CodigoConductor = '".$Chofer."',
			CodigoCadena_ = '".$CodigoCadena."'";
	...
	if ($resultCabecera == 1 && $resultLinea == 1) {
		...
		if ($Bloqueo==0) {
			//DEVOLUCIONES
			if ($DevolucionWallet==0) {
				$query = "SELECT     LineasAlbaranCliente.EjercicioPedido, LineasAlbaranCliente.SeriePedido, LineasAlbaranCliente.NumeroPedido, LineasAlbaranCliente.EjercicioAlbaran, LineasAlbaranCliente.SerieAlbaran, 
								LineasAlbaranCliente.NumeroAlbaran, LineasAlbaranCliente.ImporteLiquido, LineasAlbaranCliente.BLM_ImportePagadoB2C, 
								LineasAlbaranCliente.BLM_ImportePagadoB2C - LineasAlbaranCliente.ImporteLiquido AS ImporteDevolucion, LineasAlbaranCliente.BLM_CodigoOperacionB2C
						FROM            LineasAlbaranCliente 
						INNER JOIN CabeceraAlbaranCliente ON
						CabeceraAlbaranCliente.CodigoEmpresa=LineasAlbaranCliente.CodigoEmpresa
						AND
						CabeceraAlbaranCliente.EjercicioAlbaran=LineasAlbaranCliente.EjercicioAlbaran
						AND
						CabeceraAlbaranCliente.SerieAlbaran=LineasAlbaranCliente.SerieAlbaran
						AND
						CabeceraAlbaranCliente.NumeroAlbaran=LineasAlbaranCliente.NumeroAlbaran
						
						LEFT OUTER JOIN
								BLM_B2C_Devoluciones ON LineasAlbaranCliente.EjercicioAlbaran = BLM_B2C_Devoluciones.EjercicioAlbaran AND LineasAlbaranCliente.SerieAlbaran = BLM_B2C_Devoluciones.SerieAlbaran AND 
								LineasAlbaranCliente.NumeroAlbaran = BLM_B2C_Devoluciones.NumeroAlbaran
						WHERE        (LineasAlbaranCliente.CodigoEmpresa = 3) AND (LineasAlbaranCliente.EjercicioAlbaran = ".$EjercicioAlbaran.") 
								AND (LineasAlbaranCliente.FechaAlbaran >= CONVERT(DATETIME, '2020-07-01 00:00:00', 102)) AND 
								(LineasAlbaranCliente.NumeroPedido > 0) AND (LineasAlbaranCliente.SerieAlbaran = '".$SerieAlbaran."') 
								AND (LineasAlbaranCliente.NumeroAlbaran = ".$NumeroAlbaran.") AND (LineasAlbaranCliente.BLM_ImportePagadoB2C - LineasAlbaranCliente.ImporteLiquido > 0) AND (LineasAlbaranCliente.BLM_ImportePagadoB2C <> 0)
								AND CabeceraAlbaranCliente.CodigoTipoEfecto <>15
								AND (BLM_B2C_Devoluciones.EjercicioAlbaran IS NULL) AND CabeceraAlbaranCliente.Bloqueo=0";
			} else {
				$query = "SELECT     LineasAlbaranCliente.EjercicioPedido, LineasAlbaranCliente.SeriePedido, LineasAlbaranCliente.NumeroPedido, LineasAlbaranCliente.EjercicioAlbaran, LineasAlbaranCliente.SerieAlbaran, 
									LineasAlbaranCliente.NumeroAlbaran, LineasAlbaranCliente.ImporteLiquido, LineasAlbaranCliente.BLM_ImportePagadoB2C, 
									LineasAlbaranCliente.BLM_ImportePagadoB2C - LineasAlbaranCliente.ImporteLiquido AS ImporteDevolucion, LineasAlbaranCliente.BLM_CodigoOperacionB2C
						...
						LEFT OUTER JOIN
									BLM_B2C_DevolucionesWallet ON LineasAlbaranCliente.EjercicioAlbaran = BLM_B2C_DevolucionesWallet.EjercicioAlbaran 
									AND LineasAlbaranCliente.SerieAlbaran = BLM_B2C_DevolucionesWallet.SerieAlbaran AND 
									LineasAlbaranCliente.NumeroAlbaran = BLM_B2C_DevolucionesWallet.NumeroAlbaran
						WHERE        (LineasAlbaranCliente.CodigoEmpresa = 3) AND (LineasAlbaranCliente.EjercicioAlbaran = ".$EjercicioAlbaran.") 
								AND (LineasAlbaranCliente.FechaAlbaran >= CONVERT(DATETIME, '2020-07-01 00:00:00', 102)) AND 
									(LineasAlbaranCliente.NumeroPedido > 0) AND (LineasAlbaranCliente.SerieAlbaran = '".$SerieAlbaran."') 
								AND (LineasAlbaranCliente.NumeroAlbaran = ".$NumeroAlbaran.") AND (LineasAlbaranCliente.BLM_ImportePagadoB2C - LineasAlbaranCliente.ImporteLiquido > 0) AND (LineasAlbaranCliente.BLM_ImportePagadoB2C <> 0)
								AND CabeceraAlbaranCliente.CodigoTipoEfecto <>15
								AND (BLM_B2C_DevolucionesWallet.EjercicioAlbaran IS NULL) AND CabeceraAlbaranCliente.Bloqueo=0";
			}
			$registro = mssql_query($query);
			while ($row = mssql_fetch_array($registro, MSSQL_ASSOC)){
				$row2 = array_map('utf8_encode', $row);
			...
			if(mssql_num_rows($registro) > 0) {
				//COMPROBAR QUE EL ALBARAN NO ESTA BLOQUEADO
				$query="SELECT EjercicioAlbaran, SerieAlbaran, NumeroAlbaran,StatusFacturado FROM CabeceraAlbaranCliente 
					WHERE CodigoEmpresa=3 and EjercicioAlbaran=".$EjercicioAlbaran." AND SerieAlbaran='".$SerieAlbaran."' AND NumeroAlbaran=".$NumeroAlbaran." AND Bloqueo=0";
				//	 echo $query;die;
				$registro = mssql_query($query);
				while ($row = mssql_fetch_array($registro, MSSQL_ASSOC)){
					$row2 = array_map('utf8_encode', $row);
				...
				if(mssql_num_rows($registro) > 0) {
					//CREAR DEVOLUCION
					if ($DevolucionWallet==0) {
						$query = "INSERT INTO BLM_B2C_Devoluciones (EjercicioPedido, SeriePedido, NumeroPedido, EjercicioAlbaran, SerieAlbaran, NumeroAlbaran, ImporteAlbaran, ImportePagado, BLM_ImporteDevolucion, BLM_CodigoOperacionB2C)
					SELECT     LineasAlbaranCliente.EjercicioPedido, LineasAlbaranCliente.SeriePedido, LineasAlbaranCliente.NumeroPedido, LineasAlbaranCliente.EjercicioAlbaran, LineasAlbaranCliente.SerieAlbaran, 
									LineasAlbaranCliente.NumeroAlbaran, LineasAlbaranCliente.ImporteLiquido, LineasAlbaranCliente.BLM_ImportePagadoB2C, 
									LineasAlbaranCliente.BLM_ImportePagadoB2C - LineasAlbaranCliente.ImporteLiquido AS ImporteDevolucion, LineasAlbaranCliente.BLM_CodigoOperacionB2C
							...
							WHERE        (LineasAlbaranCliente.CodigoEmpresa = 3) AND (LineasAlbaranCliente.EjercicioAlbaran = ".$EjercicioAlbaran.") 
									...
									(LineasAlbaranCliente.NumeroPedido > 0) AND (LineasAlbaranCliente.SerieAlbaran = '".$SerieAlbaran."') 
									AND (LineasAlbaranCliente.NumeroAlbaran = ".$NumeroAlbaran.") AND (LineasAlbaranCliente.BLM_ImportePagadoB2C - LineasAlbaranCliente.ImporteLiquido > 0) AND (LineasAlbaranCliente.BLM_ImportePagadoB2C <> 0)
									AND (BLM_B2C_Devoluciones.EjercicioAlbaran IS NULL)";
					} else {
						$query = "INSERT INTO BLM_B2C_DevolucionesWallet (EjercicioPedido, SeriePedido, NumeroPedido, EjercicioAlbaran, SerieAlbaran, NumeroAlbaran, ImporteAlbaran, ImportePagado, BLM_ImporteDevolucion, BLM_CodigoOperacionB2C)
						SELECT     LineasAlbaranCliente.EjercicioPedido, LineasAlbaranCliente.SeriePedido, LineasAlbaranCliente.NumeroPedido, LineasAlbaranCliente.EjercicioAlbaran, LineasAlbaranCliente.SerieAlbaran, 
										LineasAlbaranCliente.NumeroAlbaran, LineasAlbaranCliente.ImporteLiquido, LineasAlbaranCliente.BLM_ImportePagadoB2C, 
										LineasAlbaranCliente.BLM_ImportePagadoB2C - LineasAlbaranCliente.ImporteLiquido AS ImporteDevolucion, LineasAlbaranCliente.BLM_CodigoOperacionB2C
								FROM            LineasAlbaranCliente LEFT OUTER JOIN
								...
								WHERE        (LineasAlbaranCliente.CodigoEmpresa = 3) AND (LineasAlbaranCliente.EjercicioAlbaran = ".$EjercicioAlbaran.") 
										...
										(LineasAlbaranCliente.NumeroPedido > 0) AND (LineasAlbaranCliente.SerieAlbaran = '".$SerieAlbaran."') 
										AND (LineasAlbaranCliente.NumeroAlbaran = ".$NumeroAlbaran.") AND (LineasAlbaranCliente.BLM_ImportePagadoB2C - LineasAlbaranCliente.ImporteLiquido > 0) AND (LineasAlbaranCliente.BLM_ImportePagadoB2C <> 0)
										AND (BLM_B2C_DevolucionesWallet.EjercicioAlbaran IS NULL)";
					}
					$registro = mssql_query($query);
					if($registro == 0) {
						$results[] = [
						'IdAlbaran' => $IdAlbaran,
						'ResultCode' => 1,
						'ResultText' => ''.$tiporesult.' Fail. Devolución no creada'];
						continue;
					}
		...
		if ($Bloqueo==0) {
			...
			$query = "SELECT     LineasAlbaranCliente.EjercicioPedido, LineasAlbaranCliente.SeriePedido, LineasAlbaranCliente.NumeroPedido, LineasAlbaranCliente.EjercicioAlbaran, LineasAlbaranCliente.SerieAlbaran, 
						...
						LEFT OUTER JOIN
								BLM_B2C_DevolucionesOney ON LineasAlbaranCliente.EjercicioAlbaran = BLM_B2C_DevolucionesOney.EjercicioAlbaran AND LineasAlbaranCliente.SerieAlbaran = BLM_B2C_DevolucionesOney.SerieAlbaran AND 
								LineasAlbaranCliente.NumeroAlbaran = BLM_B2C_DevolucionesOney.NumeroAlbaran
						WHERE        (LineasAlbaranCliente.CodigoEmpresa = 3) AND (LineasAlbaranCliente.EjercicioAlbaran = ".$EjercicioAlbaran.") 
								AND (LineasAlbaranCliente.FechaAlbaran >= CONVERT(DATETIME, '2020-07-01 00:00:00', 102)) AND 
								(LineasAlbaranCliente.NumeroPedido > 0) AND (LineasAlbaranCliente.SerieAlbaran = '".$SerieAlbaran."') 
								AND (LineasAlbaranCliente.NumeroAlbaran = ".$NumeroAlbaran.") AND (LineasAlbaranCliente.BLM_ImportePagadoB2C - LineasAlbaranCliente.ImporteLiquido > 0) AND (LineasAlbaranCliente.BLM_ImportePagadoB2C <> 0)
								AND CabeceraAlbaranCliente.CodigoTipoEfecto =15
								AND (BLM_B2C_DevolucionesOney.EjercicioAlbaran IS NULL) AND CabeceraAlbaranCliente.Bloqueo=0";
			//echo $query;die;							
			$registro = mssql_query($query);
			while ($row = mssql_fetch_array($registro, MSSQL_ASSOC)){
				$row2 = array_map('utf8_encode', $row);
			...
			if(mssql_num_rows($registro) > 0) {
				...
				if(mssql_num_rows($registro) > 0) {
				//echo "pasa oney1";
					//CREAR DEVOLUCION ONEY
					$query = "INSERT INTO BLM_B2C_DevolucionesOney (EjercicioPedido, SeriePedido, NumeroPedido, EjercicioAlbaran, SerieAlbaran, NumeroAlbaran, ImporteAlbaran, ImportePagado, BLM_ImporteDevolucion, BLM_CodigoOperacionB2C)
					SELECT     LineasAlbaranCliente.EjercicioPedido, LineasAlbaranCliente.SeriePedido, LineasAlbaranCliente.NumeroPedido, LineasAlbaranCliente.EjercicioAlbaran, LineasAlbaranCliente.SerieAlbaran, 
									LineasAlbaranCliente.NumeroAlbaran, LineasAlbaranCliente.ImporteLiquido, LineasAlbaranCliente.BLM_ImportePagadoB2C, 
									LineasAlbaranCliente.BLM_ImportePagadoB2C - LineasAlbaranCliente.ImporteLiquido AS ImporteDevolucion, LineasAlbaranCliente.BLM_CodigoOperacionB2C
							FROM            LineasAlbaranCliente LEFT OUTER JOIN
							...
							WHERE        (LineasAlbaranCliente.CodigoEmpresa = 3) AND (LineasAlbaranCliente.EjercicioAlbaran = ".$EjercicioAlbaran.") 
									...
									(LineasAlbaranCliente.NumeroPedido > 0) AND (LineasAlbaranCliente.SerieAlbaran = '".$SerieAlbaran."') 
									AND (LineasAlbaranCliente.NumeroAlbaran = ".$NumeroAlbaran.") AND (LineasAlbaranCliente.BLM_ImportePagadoB2C - LineasAlbaranCliente.ImporteLiquido > 0) AND (LineasAlbaranCliente.BLM_ImportePagadoB2C <> 0)
									AND (BLM_B2C_DevolucionesOney.EjercicioAlbaran IS NULL)";
					//echo $query;die;
					$registro = mssql_query($query);
					if($registro == 0) {
						$results[] = [
...
function print_json($response) {
    ...
    //TrazaJR(" DELIVERYHEPTAN_RESPUESTAJR", json_encode($response, JSON_UNESCAPED_UNICODE));
}
?>
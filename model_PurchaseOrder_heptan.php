<?php 
 ...
 class model_PurchaseOrder_heptan_class extends db_model {
  ...
  // Esta funcion sera llamada al momento de usar el metodo POST
    function post() {
        /*echo sprintf("INSERT INTO %s (CodigoEmpresa, BLM_IdCompraBases, IdDelegacion, CodigoArticulo, Fecha, Litros, HorarioEntrega, BLM_ObservacionesBases, N_Compra, BLM_Heptan_sync) VALUES (%s)",
            $this->entity,
            $this->data);*/
        return $this->set_query(sprintf("INSERT INTO %s (CodigoEmpresa, BLM_IdCompraBases, IdDelegacion, CodigoArticulo, Fecha, Litros, HorarioEntrega, BLM_ObservacionesBases, N_Compra, BLM_NumeroPedidoHeptan, BLM_IdPedidoVenta , BLM_Heptan_sync) VALUES (%s)",
            $this->entity,
            $this->data));
  ...
  function put($IdCompra) {      
   return $this->set_query(sprintf("
    ...
    SET 
     %s 
    WHERE 
     BLM_IDCompraBases = CONVERT(uniqueidentifier, '$IdCompra')", 
     $this->entity,
     $this->data
    )
   );

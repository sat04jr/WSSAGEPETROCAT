...
include 'model_CreditCustomersJR.php';
...
$select = "RiesgosPetrocat.CifDni, RiesgosPetrocat.NumeroCC, RiesgosPetrocat.ImporteAval AS RiesgoAval, RiesgosPetrocat.RiesgoCC AS RiesgoCCConcedido, 
case when RiesgosPetrocat.FechaAprobacion is null then '' else 
	RIGHT('00'+CAST(datepart (dd, RiesgosPetrocat.FechaAprobacion)  AS VARCHAR(2)),2) +'/'+RIGHT('00'+CAST(datepart (mm, RiesgosPetrocat.FechaAprobacion)  AS VARCHAR(2)),2)
+'/'+str(datepart (yy, RiesgosPetrocat.FechaAprobacion) ,4) end  AS FechaAprobacionCC,   
case when RiesgosPetrocat.FechaVencimiento1 is null then '' else 
	RIGHT('00'+CAST(datepart (dd, RiesgosPetrocat.FechaVencimiento1)  AS VARCHAR(2)),2) +'/'+RIGHT('00'+CAST(datepart (mm, RiesgosPetrocat.FechaVencimiento1)  AS VARCHAR(2)),2)
+'/'+str(datepart (yy, RiesgosPetrocat.FechaVencimiento1) ,4) end  as FechaVencimientoCC,  RiesgosPetrocat.RiesgoMaximo AS RiesgoPetrocatConcedido, 
case when RiesgosPetrocat.fechaaval is null then '' else 
	RIGHT('00'+CAST(datepart (dd, RiesgosPetrocat.fechaaval)  AS VARCHAR(2)),2) +'/'+RIGHT('00'+CAST(datepart (mm, RiesgosPetrocat.fechaaval)  AS VARCHAR(2)),2)
+'/'+str(datepart (yy, RiesgosPetrocat.fechaaval) ,4) end  as FechaVencimientoAval, RiesgosPetrocat.BLM_RAZONRiesgoCCCero AS RazonRiesgoCCCero ";
...
function get_obj() {
	$object = new model_creditcustomersJR_class;
	return $object;
}
...
function dateTransform($data) {
    foreach ($data as $key => $riesgos) {
		echo var_dump($riesgos);
		//FechaAprobacionCC
        if($riesgos['FechaAprobacionCC1'] != '') {
        ...
        if($riesgos['FechaVencimientoCC1'] != '') {
		...
		//DEVUELVE UN FORMATO INCORRECTO JUN 08 1984
        if($riesgos['FechaVencimientoAval1'] != '') {

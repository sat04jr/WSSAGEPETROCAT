...
//Traza(" FUELS", "heptanStatus=".$heptanStatus." block=".$block." tipoTarifa=".$tipoTarifa);

//DISMINUIR BLOCK HASTA 45 SEGUNDOS
if ($block>65) {
	$block = 65;
}
//por defecto es 0
...
switch ($tipoTarifa) {
    case 2:
        ...
        $fromTarifas1 = "TarifaPrecio WITH (nolock) 
		INNER JOIN Comisionistas WITH (nolock) ON 1 = Comisionistas.CodigoEmpresa AND TarifaPrecio.Tarifa = Comisionistas.BLM_TarifaPrecioMinimo AND Comisionistas.BLM_TarifaPrecioMinimo <> '0' 
		INNER JOIN Tarifas WITH (nolock) ON Comisionistas.BLM_TarifaPrecioMinimo = Tarifas.Tarifa 
		AND 2 = Tarifas.CodigoEmpresa INNER JOIN BLM_PR120_ConversionArticulos WITH (nolock) ON TarifaPrecio.CodigoEmpresa = BLM_PR120_ConversionArticulos.CodigoEmpresa 
		AND TarifaPrecio.CodigoArticulo = BLM_PR120_ConversionArticulos.CodigoArticulo AND BLM_PR120_ConversionArticulos.BLM_EnvioHeptan = - 1 
		INNER JOIN Delegaciones WITH (nolock) ON CAST(Comisionistas.CodigoComisionista AS varchar(10)) = Delegaciones.IdDelegacion 
		AND Comisionistas.CodigoEmpresa = Delegaciones.CodigoEmpresa AND -1 = Delegaciones.BLM_DPMobility";
        break;
    case 0:
        ...
        $fromTarifas2 = "BLM_PR266_TarifasBases WITH (nolock) INNER JOIN Tarifas WITH (nolock) ON BLM_PR266_TarifasBases.Tarifa = Tarifas.Tarifa AND 2 = Tarifas.CodigoEmpresa 
		INNER JOIN TarifaPrecio WITH (nolock) ON Tarifas.CodigoEmpresa = TarifaPrecio.CodigoEmpresa AND Tarifas.Tarifa = TarifaPrecio.Tarifa 
		INNER JOIN BLM_PR120_ConversionArticulos WITH (nolock) ON TarifaPrecio.CodigoEmpresa = BLM_PR120_ConversionArticulos.CodigoEmpresa 
		AND TarifaPrecio.CodigoArticulo = BLM_PR120_ConversionArticulos.CodigoArticulo AND BLM_PR120_ConversionArticulos.BLM_EnvioHeptan = - 1 
		INNER JOIN Delegaciones WITH (nolock) ON CAST(BLM_PR266_TarifasBases.CodigoComisionista AS varchar(10)) = Delegaciones.IdDelegacion AND 1 = Delegaciones.CodigoEmpresa AND -1 = Delegaciones.BLM_DPMobility";
    ...
    case 1:
		...
		$fromTarifas3 = "BLM_PV_TarifasClienteFinal WITH (nolock) INNER JOIN Tarifas ON BLM_PV_TarifasClienteFinal.BLM_CodigoTarifa1 = Tarifas.Tarifa AND 2 = Tarifas.CodigoEmpresa AND -1 = Tarifas.BLM_TarifaCliHeptan 
		INNER JOIN TarifaPrecio WITH (nolock) ON BLM_PV_TarifasClienteFinal.CodigoEmpresa = TarifaPrecio.CodigoEmpresa AND BLM_PV_TarifasClienteFinal.BLM_CodigoTarifa1 = TarifaPrecio.Tarifa 
		inner join  BLM_PR120_ConversionArticulos WITH (nolock) ON TarifaPrecio.CodigoEmpresa = BLM_PR120_ConversionArticulos.CodigoEmpresa 
		AND TarifaPrecio.CodigoArticulo = BLM_PR120_ConversionArticulos.CodigoArticulo AND BLM_PR120_ConversionArticulos.BLM_EnvioHeptan = - 1";
        $where .= " AND FechaInicio <= '".$fechaString."' AND Year(FechaInicio) >= 2017";
		break;
...
$detallesQuery1 = "SELECT distinct TarifaPrecio.FechaInicio AS Fecha, Tarifas.Tarifa AS CodigoTarifa, 2 AS TipoTarifa, BLM_PR120_ConversionArticulos.BLM_CodigoArticuloBase AS CodigoArticulo, 
	TarifaPrecio.HastaUnidades1, TarifaPrecio.Precio1, TarifaPrecio.HastaUnidades2, TarifaPrecio.Precio2, TarifaPrecio.HastaUnidades3, TarifaPrecio.Precio3, TarifaPrecio.HastaUnidades4, 
	TarifaPrecio.Precio4, TarifaPrecio.HastaUnidades5, TarifaPrecio.Precio5, TarifaPrecio.HastaUnidades6, TarifaPrecio.Precio6, TarifaPrecio.HastaUnidades7, TarifaPrecio.Precio7, 
	TarifaPrecio.HastaUnidades8, TarifaPrecio.Precio8, TarifaPrecio.HastaUnidades9, TarifaPrecio.Precio9, TarifaPrecio.HastaUnidades10, TarifaPrecio.Precio10 
	FROM TarifaPrecio WITH (nolock) INNER JOIN Comisionistas WITH (nolock) ON 1 = Comisionistas.CodigoEmpresa AND TarifaPrecio.Tarifa = Comisionistas.BLM_TarifaPrecioMinimo AND Comisionistas.BLM_TarifaPrecioMinimo <> '0' 
	INNER JOIN Tarifas WITH (nolock) ON Comisionistas.BLM_TarifaPrecioMinimo = Tarifas.Tarifa AND 2 = Tarifas.CodigoEmpresa 
	INNER JOIN BLM_PR120_ConversionArticulos WITH (nolock) ON TarifaPrecio.CodigoEmpresa = BLM_PR120_ConversionArticulos.CodigoEmpresa 
	AND TarifaPrecio.CodigoArticulo = BLM_PR120_ConversionArticulos.CodigoArticulo AND BLM_PR120_ConversionArticulos.BLM_EnvioHeptan = - 1 
	INNER JOIN Delegaciones WITH (nolock) ON CAST(Comisionistas.CodigoComisionista AS varchar(10)) = Delegaciones.IdDelegacion AND Comisionistas.CodigoEmpresa = Delegaciones.CodigoEmpresa";

$detallesQuery2 = "SELECT distinct TarifaPrecio.FechaInicio AS Fecha, Tarifas.Tarifa AS CodigoTarifa, 0 AS TipoTarifa, BLM_PR120_ConversionArticulos.BLM_CodigoArticuloBase AS CodigoArticulo, 
	TarifaPrecio.HastaUnidades1, TarifaPrecio.Precio1, TarifaPrecio.HastaUnidades2, TarifaPrecio.Precio2, TarifaPrecio.HastaUnidades3, TarifaPrecio.Precio3, TarifaPrecio.HastaUnidades4, 
	TarifaPrecio.Precio4, TarifaPrecio.HastaUnidades5, TarifaPrecio.Precio5, TarifaPrecio.HastaUnidades6, TarifaPrecio.Precio6, TarifaPrecio.HastaUnidades7, TarifaPrecio.Precio7, 
	TarifaPrecio.HastaUnidades8, TarifaPrecio.Precio8, TarifaPrecio.HastaUnidades9, TarifaPrecio.Precio9, TarifaPrecio.HastaUnidades10, TarifaPrecio.Precio10 
	FROM BLM_PR266_TarifasBases WITH (nolock) INNER JOIN Tarifas WITH (nolock) ON BLM_PR266_TarifasBases.Tarifa = Tarifas.Tarifa AND 2 = Tarifas.CodigoEmpresa 
	INNER JOIN TarifaPrecio WITH (nolock) ON Tarifas.CodigoEmpresa = TarifaPrecio.CodigoEmpresa AND Tarifas.Tarifa = TarifaPrecio.Tarifa 
	INNER JOIN BLM_PR120_ConversionArticulos WITH (nolock) ON TarifaPrecio.CodigoEmpresa = BLM_PR120_ConversionArticulos.CodigoEmpresa 
	AND TarifaPrecio.CodigoArticulo = BLM_PR120_ConversionArticulos.CodigoArticulo AND BLM_PR120_ConversionArticulos.BLM_EnvioHeptan = - 1 
	INNER JOIN Delegaciones WITH (nolock) ON CAST(BLM_PR266_TarifasBases.CodigoComisionista AS varchar(10)) = Delegaciones.IdDelegacion AND 1 = Delegaciones.CodigoEmpresa";
...
$detallesQuery3 = "SELECT distinct TarifaPrecio.FechaInicio AS Fecha, Tarifas.Tarifa AS CodigoTarifa, 1 AS TipoTarifa, BLM_PR120_ConversionArticulos.BLM_CodigoArticuloBase as CodigoArticulo, 
TarifaPrecio.HastaUnidades1, TarifaPrecio.Precio1, TarifaPrecio.HastaUnidades2, TarifaPrecio.Precio2, TarifaPrecio.HastaUnidades3, TarifaPrecio.Precio3, TarifaPrecio.HastaUnidades4, TarifaPrecio.Precio4, 
TarifaPrecio.HastaUnidades5, TarifaPrecio.Precio5, TarifaPrecio.HastaUnidades6, TarifaPrecio.Precio6, TarifaPrecio.HastaUnidades7, TarifaPrecio.Precio7, TarifaPrecio.HastaUnidades8, TarifaPrecio.Precio8, 
TarifaPrecio.HastaUnidades9, TarifaPrecio.Precio9, TarifaPrecio.HastaUnidades10, TarifaPrecio.Precio10 
FROM BLM_PV_TarifasClienteFinal WITH (nolock) INNER JOIN Tarifas WITH (nolock) ON BLM_PV_TarifasClienteFinal.BLM_CodigoTarifa1 = Tarifas.Tarifa AND 2 = Tarifas.CodigoEmpresa AND -1 = Tarifas.BLM_TarifaCliHeptan 
INNER JOIN TarifaPrecio WITH (nolock) ON BLM_PV_TarifasClienteFinal.CodigoEmpresa = TarifaPrecio.CodigoEmpresa AND BLM_PV_TarifasClienteFinal.BLM_CodigoTarifa1 = TarifaPrecio.Tarifa 
INNER JOIN BLM_PR120_ConversionArticulos WITH (nolock) ON TarifaPrecio.CodigoEmpresa = BLM_PR120_ConversionArticulos.CodigoEmpresa AND TarifaPrecio.CodigoArticulo = BLM_PR120_ConversionArticulos.CodigoArticulo 
AND BLM_PR120_ConversionArticulos.BLM_EnvioHeptan = - 1";

//$whereDelegaciones = Tarifas.Tarifa = Tarifa1ªQuery;

$delegacionesQuery1 = "SELECT DISTINCT Delegaciones.IdDelegacion, Tarifas.Tarifa 
	FROM TarifaPrecio WITH (nolock) INNER JOIN Comisionistas WITH (nolock) ON 1 = Comisionistas.CodigoEmpresa 
	AND TarifaPrecio.Tarifa = Comisionistas.BLM_TarifaPrecioMinimo AND Comisionistas.BLM_TarifaPrecioMinimo <> '0' 
	INNER JOIN Tarifas WITH (nolock) ON Comisionistas.BLM_TarifaPrecioMinimo = Tarifas.Tarifa AND 2 = Tarifas.CodigoEmpresa 
	INNER JOIN BLM_PR120_ConversionArticulos WITH (nolock) ON TarifaPrecio.CodigoEmpresa = BLM_PR120_ConversionArticulos.CodigoEmpresa 
	AND TarifaPrecio.CodigoArticulo = BLM_PR120_ConversionArticulos.CodigoArticulo AND BLM_PR120_ConversionArticulos.BLM_EnvioHeptan = - 1 
	INNER JOIN Delegaciones WITH (nolock) ON CAST(Comisionistas.CodigoComisionista AS varchar(10)) = Delegaciones.IdDelegacion AND Comisionistas.CodigoEmpresa = Delegaciones.CodigoEmpresa AND -1 = Delegaciones.BLM_DPMobility";

$delegacionesQuery2 = "SELECT DISTINCT Delegaciones.IdDelegacion, Tarifas.Tarifa 
	FROM BLM_PR266_TarifasBases WITH (nolock) INNER JOIN Tarifas WITH (nolock) ON BLM_PR266_TarifasBases.Tarifa = Tarifas.Tarifa AND 2 = Tarifas.CodigoEmpresa 
	INNER JOIN TarifaPrecio WITH (nolock) ON Tarifas.CodigoEmpresa = TarifaPrecio.CodigoEmpresa AND Tarifas.Tarifa = TarifaPrecio.Tarifa 
	INNER JOIN BLM_PR120_ConversionArticulos WITH (nolock) ON TarifaPrecio.CodigoEmpresa = BLM_PR120_ConversionArticulos.CodigoEmpresa 
	AND TarifaPrecio.CodigoArticulo = BLM_PR120_ConversionArticulos.CodigoArticulo AND BLM_PR120_ConversionArticulos.BLM_EnvioHeptan = - 1 
...
$consumidoresQuery1 = "SELECT DISTINCT BLM_PV_TarifasClienteFinal.BLM_CodigoTarifa1, BLM_PV_MailClientes.CifDni 
	FROM BLM_PV_MailClientes WITH (nolock) INNER JOIN BLM_PV_TarifasClienteFinal WITH (nolock) ON BLM_PV_MailClientes.MovPosicion = BLM_PV_TarifasClienteFinal.MovOrigen 
	INNER JOIN Tarifas WITH (nolock) ON BLM_PV_TarifasClienteFinal.BLM_CodigoTarifa1 = Tarifas.Tarifa AND 2 = Tarifas.CodigoEmpresa 
	INNER JOIN TarifaPrecio WITH (nolock) ON BLM_PV_TarifasClienteFinal.CodigoEmpresa = TarifaPrecio.CodigoEmpresa AND BLM_PV_TarifasClienteFinal.BLM_CodigoTarifa1 = TarifaPrecio.Tarifa 
	INNER JOIN BLM_DatosNif WITH (nolock) ON BLM_PV_MailClientes.CifDni = BLM_DatosNif.CifDni 
	INNER JOIN  BLM_PR120_ConversionArticulos WITH (nolock) ON TarifaPrecio.CodigoEmpresa = BLM_PR120_ConversionArticulos.CodigoEmpresa 
	AND TarifaPrecio.CodigoArticulo = BLM_PR120_ConversionArticulos.CodigoArticulo AND BLM_PR120_ConversionArticulos.BLM_EnvioHeptan = - 1";
...
if(mssql_num_rows($registro) > 0) {
    ...
    foreach ($data as $key => $tarifa) {
        ...
        //DETALLES
		$fecha = dateTransformFecha($tarifa['Fecha']);
        $detallesWhere = "TarifaPrecio.FechaInicio = '".$fecha."' AND Tarifas.Tarifa = '".$tarifa['CodigoTarifa']."' AND BLM_PR120_ConversionArticulos.BLM_CodigoArticuloBase = '".$tarifa['CodigoArticulo']."'";
...
//Transforma a formato correcto la fecha y los numeros
function dateTransformFecha($fecha) {
        //FechaAlta
        if($fecha != '') {
            $strDate = strtotime($fecha);
            $badDate = date('m/d/Y', $strDate );
            $fecha = $badDate;
        }
	return $fecha;
}
...
//    Traza(" FUELS_RESPUESTA", json_encode($response, JSON_UNESCAPED_UNICODE));
}
?>
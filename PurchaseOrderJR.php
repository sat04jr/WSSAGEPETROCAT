...
$where = " (BLM_DatosNif.BLM_CodigoClienteHeptan <> '') AND (YEAR(PedidoCliente.FechaSuministro) >= 2020) AND 
						 ...
						 AND PedidoCliente.CodigoProveedor<>'4006000615'
						 AND CASE WHEN PedidoCliente.UnidadesReales =0 THEN ComprasBases.Litros ELSE PedidoCliente.UnidadesReales END >0 ";
...
$select3 = "CONVERT(char(36), ComprasBases.BLM_IDCompraBases) AS IdCompra, ComprasBases.IdDelegacion AS CodigoCanal, Clientes.CifDni, ComprasBases.CodigoArticulo, PCA.DescripcionArticulo, 
                         ...
                         pedidocliente.CodigoCentroCarga, CentrosCarga.DescripcionCentroCarga AS DescripcionCentro, CONVERT(char(36), '00000000-0000-0000-0000-000000000000') 
                         AS IdPedidoCapturado";
$tablasQuery3 = "CentrosCarga WITH (NOLOCK) RIGHT OUTER JOIN
                         ...
                         BLM_DatosNif WITH (NOLOCK) INNER JOIN
                         Clientes WITH (NOLOCK) ON BLM_DatosNif.CifDni = Clientes.CifDni ON PCA.CodigoCliente = Clientes.CodigoCliente AND 1 = Clientes.CodigoEmpresa ON 
                         Proveedores.CodigoProveedor = pedidocliente.CodigoProveedor AND PedidoCliente.NumeroPedido = PCA.NumeroPedidoAsociado ON CentrosCarga.CodigoEmpresa = pedidocliente.CodigoEmpresa AND 
                         CentrosCarga.CodigoCentroCarga = pedidocliente.CodigoCentroCarga ";
...
if ($IdCompra != '') {
    ...
    if ($heptanStatus == 'Pending') {
        ...
        $where2 .= " AND (PCA.BLM_Heptan_sync = 0)";

<?php 
 ...
 class model_riesgos_class extends db_model {
...
//Marca el riesgo como sincronizado
    function setSincronized($idRiesgo) {
        return $this->set_query(sprintf("UPDATE %s SET %s WHERE Idriesgo = CONVERT(uniqueidentifier, '%s')",
            $this->entity,
            $this->data,
            $idRiesgo
        ));
...
function getSyncStatus($idRiesgo) {
	return $this->get_query(sprintf("SELECT BLM_Heptan_sync FROM %s WHERE Idriesgo = CONVERT(uniqueidentifier, '%s')",
        $this->entity,
        $idRiesgo
        ));
}
  ...
  function post() {
   ...
   return $this->set_query(sprintf("
    INSERT INTO %s (IDRiesgo,CifDni,RiesgoCC,NumeroCC,RiesgoPetrocat,RiesgoAval,RiesgoCCConcedido,FechaAprobacionCC,FechaVencimientoCC,RiesgoPetrocatConcedido,FechaVencimientoAval,FechaSolicitud,RazonRiesgoCCCero,TipoRiesgo,StatusSolicitudRiesgo,EMailSolicitante,DiasPrimerPlazo,DiasFijos1,FormadePago,RazonSocial,Domicilio,Codigopostal,Municipio,Provincia,Telefono) VALUES (%s)",
     $this->entity,
     $this->data
    )
   );
